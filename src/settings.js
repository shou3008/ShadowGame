...
// 幅 2r px 以下の手首・前腕も一度は切れるが、SUBJECT_ONLY の測地的再構成で
// 復元されるので腕は欠けない(オープニングの役目は成分の分離とノイズ除去だけ)。
// カメラ解像度 640x360 基準。0 にすると無効。
export const MASK_OPEN_RADIUS = 2;

// ===== 被験者シルエットの選択 =====
// segmentPerson は画面内の全員を1枚のマスクに合成する。単一被験者の実験装置なので、
// オープニング後の連結成分から「前フレームの被験者マスクと最も重なる成分」を被験者と
// して追跡し、それ以外の大きな成分(映り込んだ別人)は影ごと除去する。面積でなく重なり
// で選ぶのは、別人がカメラに近づいて被験者より大きく映っても乗り移らないようにするため。
// さらに、オープニングで千切れた手首・前腕(元マスクでは体と繋がっている細部)を
// 元マスク内の測地的再構成で復元する。別人の本体は再構成の通過を禁止するので戻らない。
// 詳細は mask.js の #selectSubject。
export const SUBJECT_ONLY = true;

// 被験者以外の成分を「別人」とみなす面積の下限(全画素数に対する比)。
// 640x360 では 0.01 ≒ 2300px。これ未満の小さな成分はノイズか被験者の千切れた部位の
// 可能性があるので、除去せず従来どおり残す。
export const INTRUDER_MIN_AREA = 0.01;
...
export const INFERENCE_CONFIG = {
  ...
  // internalResolution が低いとスコア1セルがカメラ画素の数十pxを覆うため、細い前腕は
  // セル内でスコアが薄まり、閾値が高いと途中で千切れる(腕が欠ける主因)。
  //   下げる: 影が太り、腕・白い服などが欠けにくくなるが、シルエット同士が繋がりやすくなる
  //   上げる: 橋は切れるが、細い部位・低コントラスト部位に穴があきやすくなる
  // かつては橋を切るために 0.6 にしていたが、橋の切断と別人の除去は
  // MASK_OPEN_RADIUS + SUBJECT_ONLY に任せ、ここは腕の連続性を優先して 0.5 にする。
